...
- **模块划分以 PRD 第 3 章为准**：3.1 企业管理看板五模块（AI First 概览 / 生产力监控大盘 / AI 应用情况分析 / SDLC-ROI 转化漏斗 / 消耗来源分析）、3.2 团队管理看板（同模块 by 团队）、3.3 个人看板。不擅自增删模块。
- **企业 / 团队 / 个人是账号权限层级，不是导航层级。** 产品只有一个 Dashboard，看到哪一档内容由登录账号的权限决定，用户不通过导航在三者之间切换。五个模块是**同一页里的区块**，不是各自一页（对应 PRD 关键设计 1「子母板结构：主面板定义组织大盘」）。这一条曾被理解反过，导致整个骨架档位选错，改任何导航结构前先回到这里确认。
...
| MetricStrip 项数 | 个人看板明确五张卡 | 视觉预算写「3–4 个」 | **取 5** —— 组件声明本身允许 3–5 项；规范给 3–4 的理由是「第 5 个通常是零信息凑数」，这五项各自独立承载信息，不属于该情形 |
